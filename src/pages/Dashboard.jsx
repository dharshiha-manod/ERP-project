...
function ShoppingCartIcon({ size = 18, color = "currentColor" }) {
  return (
...
function PackageIcon({ size = 18, color = "currentColor" }) {
  return (
    <svg width={size} height={size} fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
...
function ReceiptIcon({ size = 18, color = "currentColor" }) {
  return (
    <svg width={size} height={size} fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
      ...
      <path d="M16 8H8m8 4H8m8 4H8"/>
    </svg>
  );
}
function MonitorIcon({ size = 18, color = "currentColor" }) {
  return (
    <svg width={size} height={size} fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
...
function UserPlusIcon({ size = 18, color = "currentColor" }) {
  return (
    <svg width={size} height={size} fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
...
function TrendDownIcon({ size = 14 }) {
  return (
    <svg width={size} height={size} fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
...
function ChartTip({ active, payload, label }) {
  ...
  return (
    <div style={{
      ...
      {payload.map((p, i) => (
        <div key={i} style={{ color: p.color || C.accentLt }}>
...
function StatCard({ label, value, icon: Icon, iconBg, iconColor, change, changeUp, prefix = "₹ ", decimals = 0, isCount }) {
  return (
    <div style={{
      ...
      boxShadow: "0 1px 8px rgba(15,31,22,0.07)", border: `1px solid ${C.border}`,
      ...
      onMouseLeave={e => { e.currentTarget.style.transform = "none"; e.currentTarget.style.boxShadow = "0 1px 8px rgba(15,31,22,0.07)"; }}
    >
...
function Avatar({ initials }) {
  const colors = ["#2d6a4f", "#3b82f6", "#8b5cf6", "#f59e0b", "#ef4444"];
  const idx = initials.charCodeAt(0) % colors.length;
  return (
    <div style={{
      width: 34, height: 34, borderRadius: 10, background: colors[idx],
      display: "flex", alignItems: "center", justifyContent: "center",
...
function Card({ children, style = {} }) {
  return (
    <div style={{
      ...
      boxShadow: "0 1px 8px rgba(15,31,22,0.07)",
...
export default function Dashboard() {
  ...
  return (
    <div style={{
      ...
      color: C.text, padding: "28px 32px",
      ...
      {/* ── Header Banner ── */}
      <div style={{
        background: `linear-gradient(135deg, ${C.brand} 0%, ${C.brandMid} 55%, ${C.brandLt} 100%)`,
        borderRadius: 20, padding: "28px 36px", marginBottom: 28,
        display: "flex", alignItems: "center", justifyContent: "space-between",
        boxShadow: "0 8px 32px rgba(26,71,49,0.28)",
        ...
        <div style={{ position:"absolute", left:"35%", top:-30, width:120, height:120, borderRadius:"50%", background:"rgba(255,255,255,0.03)" }} />
        ...
        <div style={{ display:"flex", gap: 8, position:"relative" }}>
          {["Today","This Week","This Month","This Year"].map(f => (
            <button key={f} onClick={() => setPeriod(f)} style={{
              ...
              background: period===f ? "#fff" : "rgba(255,255,255,0.15)",
              color: period===f ? C.brand : "#fff",
              transition: "all 0.2s",
              boxShadow: period===f ? "0 2px 12px rgba(0,0,0,0.15)" : "none",
            }}>{f}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{
        ...
        <StatCard label="Total Sales"         value={328450} icon={ShoppingCartIcon} iconBg="#dcfce7" iconColor={C.brandMid} change="+18.4%" changeUp decimals={0} />
        <StatCard label="Net Profit"          value={124800} icon={ChartBarIcon}     iconBg="#eff6ff" iconColor={C.blue}     change="+12.1%" changeUp decimals={0} />
        <StatCard label="Invoice Due"         value={47300}  icon={ReceiptIcon}      iconBg="#fffbeb" iconColor={C.gold}     change="+3.2%"  changeUp={false} decimals={0} />
        <StatCard label="Total Sell Return"   value={9200}   icon={PackageIcon}      iconBg="#fef2f2" iconColor={C.red}      change="-8.5%"  changeUp decimals={0} />
      </div>
      <div style={{
        display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:16, marginBottom:28,
      }}>
        <StatCard label="Total Purchase"       value={203650} icon={PackageIcon}      iconBg="#eef2ff" iconColor={C.purple} change="+9.7%"  changeUp decimals={0} />
        <StatCard label="Purchase Due"         value={31500}  icon={BellIcon}         iconBg="#fff7ed" iconColor="#f97316" change="-2.1%"  changeUp decimals={0} />
        <StatCard label="Total Orders"         value={1284}   icon={ShoppingCartIcon} iconBg="#f0fdf4" iconColor={C.accent} change="+24%"  changeUp isCount />
        <StatCard label="Total Expense"        value={28900}  icon={ReceiptIcon}      iconBg="#faf5ff" iconColor={C.purple} change="+6.3%" changeUp={false} decimals={0} />
      </div>

      {/* ── Charts Row 1 ── */}
      <div style={{ display:"grid", gridTemplateColumns:"2fr 1fr", gap:20, marginBottom:20 }}>

        {/* Sales vs Purchase Area Chart */}
        <Card>
          <CardHeader
            title="📈 Sales vs Purchase — Last 30 Days"
            subtitle="Manodtechnologies (BL0001)"
            action={
              <span style={{ fontSize:12, color:C.muted, background:"#f0fdf4", padding:"4px 12px", borderRadius:20, fontWeight:600 }}>
                May – Jun 2026
              </span>
            }
          />
              ...
              <AreaChart data={salesData} margin={{ top:5, right:10, left:0, bottom:0 }}>
                <defs>
                  <linearGradient id="gSales" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor={C.brandMid} stopOpacity={0.25} />
                ...
                <Area type="monotone" dataKey="sales" name="Sales" stroke={C.brandMid} strokeWidth={2.5} fill="url(#gSales)" dot={false} />
                <Area type="monotone" dataKey="purchases" name="Purchases" stroke={C.blue} strokeWidth={2} fill="url(#gPurch)" dot={false} />
              </AreaChart>
            </ResponsiveContainer>
            <div style={{ display:"flex", gap:24, marginTop:12 }}>
              {[
                { label:"Total Sales",  val:"₹3,28,450", dot: C.brandMid },
                { label:"Total Purchase", val:"₹2,03,650", dot: C.blue },
                { label:"Net Profit",   val:"₹1,24,800", dot: C.accent },
              ].map(m => (
                <div key={m.label} style={{ display:"flex", alignItems:"center", gap:8 }}>
                  <div style={{ width:10, height:10, borderRadius:3, background:m.dot }} />
                  <div>
                    <div style={{ fontSize:11, color:C.muted }}>{m.label}</div>
                    <div style={{ fontWeight:800, fontSize:14, color:C.text }}>{m.val}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ padding:"14px 22px 20px" }}>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
              {quickActions.map((q, i) => (
                <button key={i} onClick={() => go(q.path)} style={{
                  display:"flex", alignItems:"center", gap:12,
                  padding:"12px 14px", borderRadius:12, border:"none",
                  background:q.bg, cursor:"pointer",
                  fontSize:13, fontWeight:700, color:q.accent,
                ...
                >
                  <div style={{
                    width:34, height:34, borderRadius:9, background:`${q.accent}18`,
                ...
                {alerts.map((a, i) => {
                  ...
                  return (
                    <div key={i} style={{
                      ...
                      background:cfg.bg, borderRadius:8, padding:"8px 10px",
                    }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20, marginBottom:20 }}>
          ...
          <div style={{ padding:"12px 0 0" }}>
            <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
                ...
                <tr>
                  {["Invoice","Customer","Amount","Status","Date"].map(h => (
                    <th key={h} style={{
                      ...
                      fontWeight:700, color:C.muted, letterSpacing:"0.3px",
              ...
              <tbody>
                {recentSales.map((s, i) => (
                  <tr key={i} style={{
                    borderBottom:`1px solid ${i < recentSales.length-1 ? C.border : "transparent"}`,
                  ...
                  >
                    <td style={{ padding:"11px 14px" }}>
                      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                        ...
                        <span style={{ color:C.brandMid, fontWeight:700, fontSize:12 }}>{s.id}</span>
                      </div>
                    </td>
                    <td style={{ padding:"11px 14px", color:C.text, fontWeight:500 }}>{s.customer}</td>
                    <td style={{ padding:"11px 14px", fontWeight:800, color:C.text }}>
        ...
        <Card>
          <CardHeader
            title="🔥 Top Products"
            action={
              <button onClick={() => go("/reports/product-sell")} style={{
                fontSize:12, fontWeight:700, color:C.brandMid,
                background:"#f0fdf4", border:"none", cursor:"pointer",
                padding:"5px 12px", borderRadius:20,
              }}>Full Report →</button>
          ...
          <div style={{ padding:"12px 0 0" }}>
            <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
              ...
              <tbody>
                {topProducts.map((p, i) => (
                  <tr key={i} style={{
                    borderBottom:`1px solid ${i < topProducts.length-1 ? C.border : "transparent"}`,
                    transition:"background 0.15s",
                  }}
                    onMouseEnter={e => e.currentTarget.style.background = "#f8fbf9"}
                    onMouseLeave={e => e.currentTarget.style.background = "transparent"}
                  >
                    <td style={{ padding:"10px 14px" }}>
                      <div style={{ fontWeight:700, color:C.text, fontSize:12 }}>{p.name}</div>
                      <div style={{ fontSize:10, color:C.muted }}>{p.sku}</div>
                    </td>
                    <td style={{ padding:"10px 14px", color:C.muted, fontWeight:600 }}>{p.sold}</td>
                    <td style={{ padding:"10px 14px", fontWeight:800, color:C.brandMid }}>
                    ...
                    <td style={{ padding:"10px 14px" }}>
                      <span style={{
                        ...
                        color: p.up ? "#16a34a" : C.red,
                        background: p.up ? "#dcfce7" : "#fee2e2",
                        padding:"3px 8px", borderRadius:20,
                      }}>
                        {p.up ? <TrendUpIcon size={11} /> : <TrendDownIcon size={11} />}
                        {p.trend}
                      </span>
                    </td>
                  </tr>
            ...
            <div style={{
              ...
              {[
                { label:"Low Stock",   val:"12", color:C.gold },
                { label:"Out of Stock",val:"3",  color:C.red },
                { label:"Total Products",val:"486",color:C.brandMid },
              ].map((m, i) => (
                <div key={i} style={{
      ...
      {/* ── Footer ── */}
      <div style={{
        textAlign:"center", color:C.muted, fontSize:12, paddingTop:8,
