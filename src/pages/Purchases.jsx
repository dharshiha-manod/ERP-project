...
const selProd=(p)=>{
  ...
  setItems(prev=>[...prev,{id:p.id||null,name:p.name,sku:p.sku||"",qty:1,unitCost:cost,discPct:0,marginPct:0,lineTotal:cost,sellingPrice:cost,isCapital:false,assetCategory:"",usefulLife:"5"}]);
...
const addManual=()=>{if(!prodSearch.trim())return;setItems(prev=>[...prev,{id:null,name:prodSearch,sku:"",qty:1,unitCost:0,discPct:0,marginPct:0,lineTotal:0,sellingPrice:0,isCapital:false,assetCategory:"",usefulLife:"5"}]);setProdSearch("");setShowProdDrop(false);};
  ...
  const toggleCapital=(i)=>{setItems(prev=>{const arr=[...prev];arr[i]={...arr[i],isCapital:!arr[i].isCapital};return arr;});};
  const updItemText=(i,field,val)=>{setItems(prev=>{const arr=[...prev];arr[i]={...arr[i],[field]:val};return arr;});};
    ...
    <div style={{fontFamily:"'Segoe UI',-apple-system,sans-serif",maxWidth:1100}}>
      ...
      <div style={{...FC,marginTop:16}}>
        ...
        <div style={{overflowX:"auto"}}>
          ...
          <thead><tr style={{background:"#f0fdf4",borderBottom:"2px solid #bbf7d0"}}>{["#","Product Name","Qty","Unit Cost","Disc %","Cost (ex disc)","Line Total","Margin %","Selling Price","Capital Asset",""].map(h=><th key={h} style={{padding:"10px",textAlign:"left",fontWeight:700,color:"#15803d",fontSize:12,whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead>
            <tbody>
              {items.length===0?<tr><td colSpan={11} style={{textAlign:"center",padding:40,color:"#9ca3af",fontSize:14}}>No products added — search above to add</td></tr>
              :items.map((item,i)=>{const cxd=item.unitCost*(1-item.discPct/100);return(<tr key={i} style={{borderBottom:"1px solid #f3f4f6"}} onMouseEnter={e=>e.currentTarget.style.background="#fafafa"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}><td style={TD}><span style={{fontWeight:600,color:"#6b7280"}}>{i+1}</span></td><td style={TD}><div style={{fontWeight:600}}>{item.name}</div>{item.sku&&<div style={{fontSize:11,color:"#9ca3af"}}>{item.sku}</div>}</td><td style={TD}><input type="number" defaultValue={item.qty} min={1} style={{...INP,width:60,padding:"6px 8px"}} onChange={e=>updItem(i,"qty",e.target.value)}/></td><td style={TD}><input type="number" defaultValue={item.unitCost} min={0} style={{...INP,width:80,padding:"6px 8px"}} onChange={e=>updItem(i,"unitCost",e.target.value)}/></td><td style={TD}><input type="number" defaultValue={item.discPct} min={0} max={100} style={{...INP,width:60,padding:"6px 8px"}} onChange={e=>updItem(i,"discPct",e.target.value)}/></td><td style={TD}>₹{cxd.toFixed(2)}</td><td style={TD}><b style={{color:"#15803d"}}>₹{item.lineTotal.toFixed(2)}</b></td><td style={TD}><input type="number" defaultValue={item.marginPct} min={0} style={{...INP,width:60,padding:"6px 8px"}} onChange={e=>updItem(i,"marginPct",e.target.value)}/></td><td style={TD}><span style={{color:"#7c3aed",fontWeight:600}}>₹{item.sellingPrice.toFixed(2)}</span></td><td style={TD}><div style={{display:"flex",flexDirection:"column",gap:4,minWidth:120}}><label style={{display:"flex",alignItems:"center",gap:5,fontSize:11,cursor:"pointer"}}><input type="checkbox" checked={!!item.isCapital} onChange={()=>toggleCapital(i)} style={{width:14,height:14,cursor:"pointer"}}/>Fixed Asset</label>{item.isCapital&&<><input placeholder="Category" defaultValue={item.assetCategory} onChange={e=>updItemText(i,"assetCategory",e.target.value)} style={{...INP,width:110,padding:"4px 6px",fontSize:11}}/><input type="number" placeholder="Life (yrs)" defaultValue={item.usefulLife} min={1} onChange={e=>updItemText(i,"usefulLife",e.target.value)} style={{...INP,width:110,padding:"4px 6px",fontSize:11}}/></>}</div></td><td style={TD}><button onClick={()=>setItems(items.filter((_,idx)=>idx!==i))} style={{background:"#fee2e2",border:"1px solid #fca5a5",borderRadius:6,color:"#dc2626",cursor:"pointer",padding:"5px 10px"}}>🗑</button></td></tr>);})}
            </tbody>
          </table>
        </div>
