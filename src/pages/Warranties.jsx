...
const SAMPLE_WARRANTIES = [
  { name: "Standard Warranty", description: "Covers manufacturing defects and hardware failures under normal use conditions.", duration: "1", durationUnit: "Years" },
  { name: "Extended Warranty", description: "Extended coverage including accidental damage and liquid damage protection.", duration: "2", durationUnit: "Years" },
  { name: "Premium Warranty", description: "Comprehensive coverage including on-site service, parts and labor for all damages.", duration: "3", durationUnit: "Years" },
  { name: "Short Term Warranty", description: "Basic coverage for electronics and accessories for short-term purchases.", duration: "6", durationUnit: "Months" },
  { name: "Battery Warranty", description: "Specific warranty covering battery degradation and replacement for portable devices.", duration: "18", durationUnit: "Months" },
  { name: "Lifetime Warranty", description: "Lifetime coverage on structural defects for tools and durable goods.", duration: "10", durationUnit: "Years" },
  { name: "Appliance Warranty", description: "Covers all major home appliances against mechanical and electrical failures.", duration: "2", durationUnit: "Years" },
  { name: "Quick Replacement", description: "Fast replacement guarantee within 30 days for defective units.", duration: "30", durationUnit: "Days" },
];
...
function exportCSV(warranties) {
  ...
  const rows = warranties.map(w => [`"${w.name}"`, `"${w.description}"`, `"${w.duration}"`, `"${w.durationUnit}"`]);
...
function exportExcel(warranties) {
  if (!warranties.length) { alert("No data to export"); return; }
  const data = warranties.map(w => ({ Name: w.name, Description: w.description, Duration: w.duration, Unit: w.durationUnit }));
...
function exportPDF(warranties) {
  ...
  autoTable(doc, {
    ...
    body: warranties.map(w => [w.name, w.description, w.duration, w.durationUnit]),
...
function printWarranties(warranties) {
  ...
  win.document.write(`<html><head><title>Warranties</title><style>
    ...
    <tbody>${warranties.map(w => `<tr><td>${w.name}</td><td>${w.description}</td><td>${w.duration}</td><td>${w.durationUnit}</td></tr>`).join("")}</tbody>
    </table></body></html>`);
...
export default function Warranties() {
  ...
  const openAdd = () => { setForm({ name: "", description: "", duration: "", durationUnit: "" }); setEditIndex(null); setShowModal(true); };
  const openEdit = (i) => { setForm({ ...warranties[i] }); setEditIndex(i); setShowModal(true); };

  const handleSave = () => {
    if (!form.name.trim()) { alert("Name is required."); return; }
    if (!form.duration || !form.durationUnit) { alert("Duration is required."); return; }
    if (editIndex !== null) {
      const updated = [...warranties]; updated[editIndex] = form; setWarranties(updated);
    } else { setWarranties([...warranties, form]); }
    setShowModal(false);
  };

  const handleDelete = (i) => { if (window.confirm("Delete this warranty?")) setWarranties(warranties.filter((_, idx) => idx !== i)); };
    ...
    <div style={{ fontFamily: "'Segoe UI', sans-serif", color: "#333" }}>
      ...
      <div style={{ background: "#fff", borderRadius: 8, padding: 24, boxShadow: "0 1px 4px rgba(0,0,0,0.08)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          ...
          <button onClick={openAdd} style={btn.green}>
            + Add
          </button>
        </div>
        ...
        <div style={modal.overlay}>
          <div style={modal.box}>
            <div style={modal.header}>
              <h5 style={{ fontWeight: 700, fontSize: 18, margin: 0 }}>{editIndex !== null ? "Edit" : "Add"} Warranty</h5>
            ...
            <div style={modal.footer}>
              <button onClick={handleSave} style={btn.savePrimary}>🖫 Save</button>
