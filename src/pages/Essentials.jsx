...
function RemindersPage() {
  ...
  return (
    <div>
      ...
      <div className="page-header">
        <div>
          <div className="ess-title">Reminders</div>
  ...
  return (
    <div className="ess-wrap">
      ...
      <div style={{ background:"#fff", borderBottom:"1px solid #e5e7eb", padding:"12px 20px 0",
                     marginBottom:0, borderRadius:"12px 12px 0 0" }}>
        <div style={{ fontSize:13, color:"#9ca3af", marginBottom:4, fontFamily:FONT_BODY }}>
          Essentials
        </div>
        <div className="ess-tabs" style={{ padding:0, borderBottom:"none" }}>
          {TABS.map(({ key, icon }) => (
            <button key={key} className={`ess-tab${activeTab===key?" active":""}`} onClick={() => setActiveTab(key)}>
              {icon} {key}
            </button>
          ))}
        </div>
